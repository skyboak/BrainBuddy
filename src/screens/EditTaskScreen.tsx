...
// This screen allows editing of an existing task
...
import { RecurrenceType, DateType, Task } from '../models/Task';
...
const EditTaskScreen: React.FC<EditTaskScreenProps> = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const loadTags = async () => {
      try {
        const existing = await TagService.getTags(userId || null);
        setAllTags(existing);
      } catch (e) {
  ...
  useEffect(() => {
    if (originalTask) {
      ...
      setUrgency(originalTask.urgency);
      setDifficulty(originalTask.difficulty);
      setDurationMinutes(originalTask.durationMinutes);
      
      // Set recurrence
      setRecurrence(originalTask.recurrence);
      setIsRecurring(originalTask.recurrence !== 'none');
      
      // Set deadline if it exists
      if (originalTask.deadline) {
        const deadlineDate = toDate(originalTask.deadline);
        if (deadlineDate) {
  ...
  const handleUpdateTask = async () => {
    ...
    try {
      ...
      const taskData: Partial<Task> = {
        ...
        difficulty,
        durationMinutes,
        deadline: isRecurring ? null : deadline, // Set deadline only for one-time tasks
        recurrence: isRecurring ? recurrence : 'none', // Set recurrence only for recurring tasks
        tags,
      };
      
      // Update task in database
      const updatedTask = await TaskService.updateTask(originalTask.id, taskData);
      await TagService.addTags(userId || null, tags);
      
      // Cancel any existing notifications for this task
      await NotificationService.cancelNotificationsForTask(originalTask.id);
      
      // Schedule new notifications if it's a one-time task with a deadline
      if (!isRecurring && deadline) {
        // Schedule reminder notification (15 minutes before)
        await NotificationService.scheduleTaskReminder(updatedTask, 15);
      ...
      navigation.navigate('TaskDetails', { taskId: originalTask.id });
    } catch (error) {
